...
class TestGummy(unittest.TestCase):
    ...
    def test_init(self):
        ...
        gd = uc.gummy(uc.TriangularDist(mode=2.2,half_width=0.73))
        self.assertTrue(gd.x == 2.2)
        self.assertTrue(abs(gd.u - np.sqrt(0.73**2/6)) < 1e-12)
        self.assertTrue(abs(gd.U - np.sqrt(0.73**2/6)) < 1e-12)
        
        gd = uc.gummy(uc.TriangularDist(mode=2.2,half_width=0.73),k=2.6)
        self.assertTrue(gd.x == 2.2)
        self.assertTrue(abs(gd.u - np.sqrt(0.73**2/6)) < 1e-12)
        self.assertTrue(abs(gd.U - 2.6*np.sqrt(0.73**2/6)) < 1e-12)
        
        gd = uc.gummy(uc.TriangularDist(mode=2.2,half_width=0.73),k=2.6,unit='m',uunit='cm')
        self.assertTrue(gd.x == 2.2)
        self.assertTrue(abs(gd.u - np.sqrt(0.73**2/6)) < 1e-12)
